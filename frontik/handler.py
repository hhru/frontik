...
from typing import TYPE_CHECKING, Any, Coroutine, List, Optional, Type, Union, overload, Callable
...
def _fail_fast_policy(fail_fast: bool, waited: bool, host: str, path: str) -> bool:
    if fail_fast and not waited:
        handler_logger.warning(
            'attempted to make NOT waited http request to %s %s with fail fast policy, turn off fail_fast', host, path
        )
        return False
...
class PageHandler(RequestHandler):
    ...
    def __init__(self, application: FrontikApplication, request: HTTPServerRequest, **kwargs: Any) -> None:
        ...
        self.text: Any = None
        self.application: FrontikApplication = None  # type: ignore
        ...
        self.statsd_client: StatsDClient|StatsDClientStub = None  # type: ignore
        ...
        if outer_timeout:
            self.timeout_checker = get_timeout_checker(
                request.headers.get(USER_AGENT_HEADER), float(outer_timeout), request.request_time
            )
    ...
    def get_str_argument(
        self, name: str, default: str| list[str] |None= _ARG_DEFAULT, path_safe: bool = True, **kwargs: Any
    ) -> str | list[str] | None:
        if path_safe:
    ...
    async def _postprocess(self) -> Any:
        ...
        postprocessed_result = await self._run_template_postprocessors(
            self._render_postprocessors, rendered_result, meta_info
        )
    ...
    def _handle_request_exception(self, e: BaseException) -> None:
        ...
        if isinstance(e, FailFastError):
            ...
            if self.log.isEnabledFor(logging.WARNING):
                ...
                self.log.warning(
                    'FailFastError: request %s failed with %s code', request_name, e.failed_result.status_code
                )
    ...
    def set_mandatory_cookie(self, name: str, value: str, domain: str|None=None, expires: str|None=None, path: str="/", expires_days: int|None=None, **kwargs: Any) -> None:
    ...
    def _execute_http_client_method(self, host: str, path: str, client_method: Callable, waited: bool) -> Future[RequestResult]:
