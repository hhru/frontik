...
class HTTPClientMockTestCase(AsyncHTTPTestCase):
    def setUp(self):
        self.common_headers = {}
        super(HTTPClientMockTestCase, self).setUp()
    ...
    def get_app_class(self):
        return get_test_app(self.get_handlers())

    def get_app(self):
        application = self.get_app_class()(app='frontik.testing')
        application.http_client = mock_http_client(application.http_client, self.get_stubs_path())
        return application

    def get_stubs_path(self):
        return None
    ...
    def fetch(self, path, query=None, **kwargs):
        ...
        return super(HTTPClientMockTestCase, self).fetch(make_url(path, **query), **kwargs)
...
def mock_http_client(http_client, stubs_path):
    http_client.routes = {}
