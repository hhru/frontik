...
def get_route_sort_key(route: BaseRoute) -> tuple:
    if not isinstance(route, APIRoute):
        return (0, 0, ())  # Non-APIRoute routes come last

    segments = [s for s in route.path_format.split('/') if s]
    has_params = any(s.startswith('{') and s.endswith('}') for s in segments)

    if not has_params:
        # Non-param routes come first, sorted by path length (longer first)
        return -1, -len(segments), ()
    else:
        # Param routes come after, sorted by:
        # 1. Path length (longer first)
        # 2. Param flags (earlier params come later)
        param_flags = tuple((s.startswith('{') and s.endswith('}')) for s in segments)
        return 1, -len(segments), param_flags
