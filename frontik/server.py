...
def main(config_file=None):
    ...
    try:
        ...
        if options.workers != 1:
            ...
            fork_workers(partial(_run_worker, app, init_workers_count_down),
            ...
            _run_worker(app, init_workers_count_down, True)
    except Exception as e:
        log.exception('frontik application exited with exception: %s', e)
        sys.exit(1)


def _run_worker(app, init_workers_count_down, need_to_init=False):
    gc.enable()
    MDC.init('worker')
    ...
    initialize_application_task = ioloop.asyncio_loop.create_task(
        _init_app(app, ioloop, init_workers_count_down, need_to_init)
    )
...
async def _init_app(app: FrontikApplication, ioloop: BaseAsyncIOLoop, init_workers_count_down,
