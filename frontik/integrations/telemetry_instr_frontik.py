...
from opentelemetry.instrumentation.propagators import get_global_response_propagator, FuncSetter
...
# response_propagation_setter = FuncSetter(frontik.handler.PageHandler.add_header)


class ExperimentalOtelInstrumentor(BaseInstrumentor):
    patched_handlers = []
...
@contextmanager
def otel_instrumentation_ctx(experimental_instrumentor, tornado_request):
    result_dict = {
        'status': None,
        'headers': None,
    }
    ...
    tracer = experimental_instrumentor.tracer
    span = _start_span(tracer, tornado_request)

    with trace.use_span(span, end_on_exit=True):
        try:
            yield result_dict
        finally:
            _finish_span(span, result_dict)
    ...
    if tornado_request.remote_ip:
        ...
        if hasattr(tornado_request.connection, "context") and getattr(
            ...
            attrs[SpanAttributes.NET_PEER_IP] = (
                tornado_request.connection.context._orig_remote_ip
            )

    return extract_attributes_from_object(
        tornado_request, _traced_request_attrs, attrs
    )
...
def _finish_span(span, result_dict):
    status_code = result_dict['status']
    resp_headers = result_dict['headers']
