...
__timeout_counters = Counter()


def __send_stats(interval_ms):
    timeout_tracking_logger.debug('timeout stats size: %d', len(__timeout_counters))
    for data, count in __timeout_counters.items():
    ...
    __timeout_counters.clear()


if options.send_timeout_stats_interval_ms:
    __send_stats_callback = PeriodicCallback(partial(__send_stats, options.send_timeout_stats_interval_ms),
...
def get_timeout_checker(outer_caller, outer_timeout_ms, time_since_outer_request_start_ms_supplier, *,
                        threshold_ms=100):
    if options.send_timeout_stats_interval_ms and not __send_stats_callback.is_running():
        __send_stats_callback.start()
