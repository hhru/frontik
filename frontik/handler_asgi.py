...
from frontik.response import Response
...
async def execute_asgi_page(
    ...
    async def send(data):
        ...
        if data['type'] == 'http.response.start':
            ...
            chunk = data['body']
            if not json_builder.is_empty():
                if chunk != b'null':
                    raise RuntimeError('Cant have "return" and "json.put" at the same time')
                chunk = json_builder.to_bytes()
                response.headers['Content-Type'] = media_types.APPLICATION_JSON
...
def make_debug_auth_failed_response(auth_header: str) -> Response:
    return Response(status_code=http.client.UNAUTHORIZED, headers={'WWW-Authenticate': auth_header})


def make_not_accepted_response() -> Response:
    return Response(status_code=http.client.SERVICE_UNAVAILABLE)


def build_error_data(status_code: int = 500, message: Optional[str] = 'Internal Server Error') -> Response:
    ...
    return Response(status_code=status_code, headers=headers, body=data)
