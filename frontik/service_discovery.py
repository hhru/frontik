...
class UpstreamCaches:
    ...
    def initial_upstreams_caches(self):
        ...
        for upstream in self._upstream_list:
            ...
            for dc in datacenters:
                ...
                health_cache.add_listener(partial(self._update_upstreams_service, dc.lower()), True)
                health_cache.start()
    ...
    def _update_upstreams_service(self, dc, key, values):
        if values is not None:
            _, servers = consul_parser.parse_consul_health_servers_data(values)
