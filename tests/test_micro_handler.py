...
class TestMicroHandler(unittest.TestCase):
    ...
    def test_fail_on_error(self):
        response = frontik_test_app.get_page('micro_handler?fail_on_error_request=true')

        self.assertEqual(response.status_code, 403)
        self.assertEqual(response.content, b'{"fail_on_error": true}')

    def test_future_fail(self):
        response = frontik_test_app.get_page('micro_handler?fail_future=true')
        self.assertEqual(response.status_code, 500)

    def test_future_with_unknown_result_fail_on_error(self):
        response = frontik_test_app.get_page('micro_handler?fail_on_error_future=true')
