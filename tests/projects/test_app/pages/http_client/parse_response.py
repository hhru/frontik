...
@plain_router.get('/http_client/parse_response', cls=PageHandler)
...
@plain_router.post('/http_client/parse_response', cls=PageHandler)
async def post_page(handler=get_current_handler()):
    handler.json.put({'post': True})
    raise HTTPErrorWithPostprocessors(400)


@plain_router.put('/http_client/parse_response', cls=PageHandler)
async def put_page(handler=get_current_handler()):
    handler.json.put({'put': True})
    raise HTTPErrorWithPostprocessors(400)


@plain_router.delete('/http_client/parse_response', cls=PageHandler)
