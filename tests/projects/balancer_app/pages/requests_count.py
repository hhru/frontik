...
async def get_page(request: Request, http_client: HttpClient) -> str:
    ...
    upstreams[requests_count_async] = Upstream(
        requests_count_async, UpstreamConfigs({}), [get_server(request, 'normal')]
    )
    ...
    upstreams[requests_count_async].update(
        Upstream(requests_count_async, UpstreamConfigs({}), [get_server(request, 'normal')])
    )
