...
@router.put('/debug')
async def put_debug_page(content_type: str = Query(alias='type')):
...
class TestDebugFailed(FrontikTestBase):
    ...
    def test_debug_by_basic_auth_with_invalid_header(self) -> None:
        ...
        for h in invalid_headers:
            self.assert_debug_response_code('/debug_simple?debug', http.client.UNAUTHORIZED, headers={'Authorization': h})
    ...
    def test_debug_by_cookie(self):
        for param in ('debug', 'noxsl', 'notpl'):
            self.assert_debug_response_code('/debug_simple', http.client.UNAUTHORIZED, headers={'Cookie': f'{param}=true'})
