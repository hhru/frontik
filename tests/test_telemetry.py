...
def find_span(attr, value):
    return next(filter(lambda item: item.attributes.get(attr, None) == value, SPAN_STORAGE), None)
...
class TestFrontikTesting(FrontikTestCase):
    ...
    @gen_test
    async def test_parent_span(self):
